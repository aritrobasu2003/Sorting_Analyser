...
function* _merge(arr, l, m, r) {
  ...
  while (i < n1 && j < n2) {
    find_and_compare(arr, L[i].val, R[j].val);
    if (L[i].val < R[j].val) {
      find_and_swap(arr, arr[k].val, L[i].val);
      arr[k] = L[i];
      k++;
      i++;
    }
    //increment after the value given
      ...
      find_and_swap(arr, arr[k].val, R[j].val);
...
function* _quickSortLomuto(arr, left, right) {
  ...
  //we need the pivot so wee run this till we the pivot
  while (!result.done) {
...
function find_and_compare(arr, val1, val2) {
...
function find_and_swap(arr, val1, val2) {
  //find the loc of val in arr and set the color
  index_1 = 0;
  index_2 = 0;
  for (var i = 0; i < arr.length; i++) {
